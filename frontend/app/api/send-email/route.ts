...
export async function POST(request: Request) {
    try {
        ...
        if (type === 'forgot-password') {
            ...
            html = `
                <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; max-width: 500px; margin: 0 auto; padding: 40px; border: 1px solid #e5e7eb; border-radius: 20px; background-color: #ffffff;">
                    ...
                    <div style="text-align: center; margin: 32px 0;">
                        <a href="${cleanResetURL}" style="background-color: #3b82f6; color: #ffffff; padding: 14px 28px; border-radius: 12px; text-decoration: none; font-weight: 600; font-size: 16px; display: inline-block; transition: all 0.2s;">Reset Password</a>
                    </div>
            ...
            const cleanVerificationURL = dompurify.sanitize(verificationURL || '');
            html = `<div>Verification Link: ${cleanVerificationURL}</div>`;
            text = `Verification Link: ${cleanVerificationURL}`;
        }
